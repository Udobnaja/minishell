...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/27 22:48:19 by alavrukh          #+#    #+#             */
/*   Updated: 2025/10/27 22:48:21 by alavrukh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "executor_internal.h"

static t_exec_status	cd_print_error_and_exit(t_exec_status status, char *cmd,
		int errno_val)
{
...
chdir a system call that actually changes the current
directory of the process to target
 */
static t_exec_status	cd_go_path(t_shell *sh, const char *target)
{
	...
	oldpwd = u_getcwd();
	if (chdir(target) != 0)
		return (cd_print_error_and_exit(EXEC_ERR_GEN, "cd", errno));
	if (oldpwd && env_set(sh->env_store, "OLDPWD", oldpwd) != ENV_OK)
		return (cd_print_error_and_exit(EXEC_ALLOC_ERR, NULL, 0));
	errno = 0;
	newpwd = u_getcwd();
	if (!newpwd)
		return (cd_print_error_and_exit(EXEC_ERR_GEN, "cd", errno));
...
static t_exec_status	cd_go_home(t_shell *sh)
{
	...
	if (!home)
		return (cd_print_error_and_exit(EXEC_ERR_CD, NULL, 0));
...
t_exec_status	cd(t_shell *sh, t_cmd *cmd)
{
	...
	if (exec_is_invalid_option(cmd->argv[1]))
		return (cd_print_error_and_exit(EXEC_ERR_INVALID_OPTION, cmd->argv[0],
				0));
	if (cmd->argv[2])
		return (cd_print_error_and_exit(EXEC_TOO_MANY_ARGS, cmd->argv[0], 0));
