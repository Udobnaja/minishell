...
t_exec_status			export_set_pairs(t_env_store *store, char *av,
							char *eqpos);
...
int						cmd_path(t_shell *sh, const char *name,
							char out[PATH_MAX]);
t_exec_result			exec_external_error_result(t_exec_status status,
							const char *cmd, int errno_val);
t_exec_result			exec_external_result(t_exec_status status,
							int exit_code);
t_exec_result			exec_external_sys_error(t_exec_status status,
							const char *cmd, int errno_val);
void					exec_child(const char *full, t_cmd *cmd, t_shell *sh,
							t_pipe *p);
void					exec_one_child(const char *full, t_cmd *cmd,
							t_shell *sh, t_pipeline *pl);
...
t_exec_result			exec_pipe_apply_stdio(t_pipe *p, int std_in,
							int std_out);
...
void					exec_run_buildin(t_shell *sh, t_cmd *cmd, pid_t *pids,
							t_pipeline *pl);
void					exec_cmd_path(t_pipe *p, t_cmd *cmd,
							char path[PATH_MAX]);
...
int						cmd_path(t_shell *sh, const char *name,
							char out[PATH_MAX]);
t_exec_result			execute_builtin(t_shell *sh, t_cmd *cmd);
t_exec_status	exec_smpl_builtin(t_shell *sh, t_cmd *cmd);
