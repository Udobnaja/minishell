...
static t_parser_status	prs_smpl_redirect_to_pipe(const t_token_type type,
							const t_word *word, t_shell *sh, t_cmd *cmd);
static t_parser_status	prs_heredoc_redirect_to_pipe(t_cmd *cmd, t_shell *sh,
							size_t *cur_heredoc);

t_parser_status	prs_redirect_to_pipe(t_token_node *token_node, t_shell *sh,
		t_cmd *cmd, size_t *cur_heredoc)
{
	if (prs_is_simple_redirect(token_node->token->type))
		return (prs_smpl_redirect_to_pipe(token_node->token->type,
				&token_node->next->token->word, sh, cmd));
	return (prs_heredoc_redirect_to_pipe(cmd, sh, cur_heredoc));
}
