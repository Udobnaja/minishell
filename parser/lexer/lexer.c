...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/27 21:50:07 by audobnai          #+#    #+#             */
/*   Updated: 2025/10/27 21:50:08 by audobnai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lexer_internal.h"

static t_lex_result	lex_ok(void);
...
t_lex_result	lex_tokenize(const char *str, t_token_list	*token_list)
{
	...
	while (str[i])
	{
		...
		if (token_result.status != LEX_OK)
			return (lex_err(token_result.status,
					token_result.payload.error.invalid_char));
