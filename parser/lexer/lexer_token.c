...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/27 21:49:59 by audobnai          #+#    #+#             */
/*   Updated: 2025/10/27 21:50:00 by audobnai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lexer_internal.h"

static t_lex_status			init_word_token(
								t_token *token, size_t pieces_count);
...
static t_lex_token_result	lex_create_operator_token(const char *str)
{
	...
	{
		lex_free_token(token);
		return (lex_token_err(parse_res.status,
				parse_res.payload.invalid_char));
	}
	return (lex_token_ok(token, parse_res.payload.consumed));
}
